 ...
 * @author bq8222
 */
public class DeleteSpeciesServ extends HttpServlet {
    ...
    throws ServletException, IOException {
       String id=request.getParameter("id"); //get username
         ...
         //use the dataAccess object to invoke the insertUser method to add a user to the database
          boolean isUserInserted= dataAccess.deleteSighting(id);
          //check if records are available
            if(isUserInserted){
               //record is inserted
                request.setAttribute("successMsg", "Sighting is deleted successfully");
            }   else
              //user not inserted
              request.setAttribute("errorMsg","Failed to delete data into the database");  
